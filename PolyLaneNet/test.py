...
def test(model, test_loader, evaluator, exp_root, cfg, view, epoch, max_batches=None, verbose=True):
    ...
    imgs = torch.tensor(imgs)[:3].to(device)
    outputs = model(imgs)
    np.save('outputs1', outputs)
    ...
    with torch.no_grad():
        for idx, (images, labels, img_idxs) in enumerate(test_loader):
            ...
            np.save('outputs1', outputs)
            criterion(outputs, labels, **criterion_parameters)
            #t = time() - t0
            ...
            np.save('images', images.cpu().numpy())
