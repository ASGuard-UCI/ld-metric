...
def warp_coord3(M, coord):
    warped_coord = np.stack([warp_coord(M, c) for c in coord])
    return warped_coord


def warp_coord2(M, coord):

    if M.shape[0] == 3:
        warped_coord = np.dot(coord, M[:2, :2].T) + M[:2, 2]
        warped_coord /= np.dot(coord, M[2, :2]).reshape(-1, 1) + M[2, 2]
    else:
        warped_coord = np.dot(coord, M[:2, :2].T) + M[:, 2]
    return warped_coord
...
def get_camera_points(trn, _points):
    points = np.zeros((_points.shape[0], 3))

    points[:, :2] = _points
    #points[:, 1] *= 0.7
    points[:, 2] = 1
    out = np.dot(trn, points.T)
    px = out[0] / out[2]
    py = out[1] / out[2]

    return np.array([px, py]).T

def get_bev_points(trn, _points):
    points = np.zeros((_points.shape[0], 3))
