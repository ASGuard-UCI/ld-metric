...
num_pts = 50#MODEL_PATH_DISTANCE
...
def vander():
    mat = np.zeros((num_pts, POLYFIT_DEGREE-1))
...
def postprocess(model_out):
    ...
    path_stds = softplus(model_out[path_start + MODEL_PATH_DISTANCE: path_start + MODEL_PATH_DISTANCE + num_pts])
    left_stds = softplus(model_out[left_start + MODEL_PATH_DISTANCE: left_start + MODEL_PATH_DISTANCE + num_pts])
    right_stds = softplus(model_out[right_start + MODEL_PATH_DISTANCE: right_start + MODEL_PATH_DISTANCE + num_pts])
    ...
    #path_prob = 1.
    left_prob = sigmoid(model_out[left_start + MODEL_PATH_DISTANCE * 2])
    right_prob = sigmoid(model_out[right_start + MODEL_PATH_DISTANCE * 2])
