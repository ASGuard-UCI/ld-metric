...
def create_mock_driving(n_frames=20, speed_ms=20):
    df_log = pd.DataFrame()
    df_log['t'] = np.arange(0, (n_frames) * 0.05, 0.05) # 20Hz
    df_log['speed'] = speed_ms
    df_log['mph'] = speed_ms * 2.23694
    df_log['steering_angle'] = 0
    df_log['lateral_shift'] = 0
    df_log['longitude_shift'] = df_log['t'] * speed_ms
    df_log['yaw'] = 0
    df_log['t_diff'] = df_log['t'].diff().fillna(0)
    df_log['distance'] = df_log['longitude_shift'].diff().fillna(0)
    return df_log
