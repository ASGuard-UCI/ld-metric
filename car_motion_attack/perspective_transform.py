...
from car_motion_attack.utils import warp_coord2
...
class PerspectiveTransform:
    ...
    def get_shifted_roatated_points(self, camera_points):

        bev_points = warp_coord2(self.mtx_src2dist, camera_points)
        lat_shift = self.lateral_shift * PIXELS_PER_METER * self.scale
        lon_shift = self.longitudinal_shift * PIXELS_PER_METER * self.scale

        mtx_lateral_shift = np.float32([[1, 0, lat_shift],
                                        [0, 1, lon_shift]])
        shifted_bev_pt = warp_coord2(mtx_lateral_shift, bev_points)

        cam_origin_shifted = (self.cam_origin[1] - lat_shift, self.cam_origin[0] - lon_shift)

        mtx_rotation = cv2.getRotationMatrix2D(cam_origin_shifted, self.yaw_diff, 1)

        shifted_rotated_bev_pt = warp_coord2(mtx_rotation, shifted_bev_pt)

        shifted_rotated_camera_pt = warp_coord2(self.mtx_dist2src, shifted_rotated_bev_pt)
    
        return shifted_rotated_camera_pt
