...
class LaneEval2(object):
    lr = LinearRegression()
    #pixel_thresh = 20
    #pt_thresh = 0.85

    def __init__(self, pixel_thresh=20, pt_thresh=0.85):
    ...
    def bench_one_submit(self, pred_file, gt_file):
        try:
            json_pred = [json.loads(line) for line in open(pred_file).readlines()]
        except BaseException as e:
            raise Exception('Fail to load json file of the prediction.')
        ...
        for pred in json_pred:
            ...
            y_samples = gt['h_samples']
            try:
                a, p, n = self.bench(pred_lanes, gt_lanes, y_samples, run_time)
            except BaseException as e:
                raise Exception('Format of lanes error.')
            accuracy += a
            fp += p
